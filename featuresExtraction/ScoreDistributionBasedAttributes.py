...
import math
import scipy.stats.mstats.ttest_ind as ttest_ind
...
class ScoreDistributionBasedAttributes :
    ...
    def getScoreDistributionBasedAttributes(self, unlabeledSamplesDataset, labeledSamplesDataset, 
                                            currentIterationIndex, evaluationResultsPerSetAndInteration,
                                            unifiedDatasetEvaulationResults, targetClassIndex, type_, properties) :
        ...
        last_key_averageingScoreDistributionsPerIteration = -1
        multiplicationScoreDistributionsPerIteration = {}
        last_key_multiplicationScoreDistributionsPerIteration = -1
        for i in range(0, len(evaluationResultsPerSetAndInteration), 1):
            ...
            for partitionIndex in evaluationResultsPerSetAndInteration.keys():
                partitionsIterationEvaulatioInfos[partitionIndex] = evaluationResultsPerSetAndInteration.get(partitionIndex).getIterationEvaluationInfo(currentIterationIndex)
             

            #next, calculate the averaging score of the iteration
            averagingScoreDistribution = self.calculateAverageClassificationResults(partitionsIterationEvaulatioInfos, labeledSamplesDataset.getNumOfClasses()) 
            averageingScoreDistributionsPerIteration[i] = averagingScoreDistribution
            last_key_averageingScoreDistributionsPerIteration = i
            multiplicationScoreDistribution = self.calculateMultiplicationClassificationResults(partitionsIterationEvaulatioInfos, labeledSamplesDataset.getNumOfClasses(), labeledSamplesDataset.getClassRatios(False)) 
            multiplicationScoreDistributionsPerIteration[i] = multiplicationScoreDistribution
            last_key_multiplicationScoreDistributionsPerIteration = i
        ...
        for pos in unifiedSetGeneralStatisticsAttributes.keys():
            currentScoreDistributionStatistics[len(currentScoreDistributionStatistics)] = unifiedSetGeneralStatisticsAttributes.get(pos)
        ...
        generalAveragingStatisticsAttributes = self.calculateGeneralScoreDistributionStatistics(unlabeledSamplesDataset, labeledSamplesDataset,
                averageingScoreDistributionsPerIteration.get(last_key_averageingScoreDistributionsPerIteration),targetClassIndex, "averaging", properties) 
        for pos in generalAveragingStatisticsAttributes.keys():
            currentScoreDistributionStatistics[len(currentScoreDistributionStatistics)] = generalAveragingStatisticsAttributes.get(pos)
        ...
        generalMultiplicationStatisticsAttributes = self.calculateGeneralScoreDistributionStatistics(unlabeledSamplesDataset, labeledSamplesDataset,
            multiplicationScoreDistributionsPerIteration.get(last_key_multiplicationScoreDistributionsPerIteration),targetClassIndex, "multiplication", properties) 
        for pos in generalMultiplicationStatisticsAttributes.keys():
            currentScoreDistributionStatistics[len(currentScoreDistributionStatistics)] = generalMultiplicationStatisticsAttributes.get(pos) 
        ...
        for i in range(0, len(allPartitionsAndDistributions)-1-2, 1):
            for j in range(0, len(allPartitionsAndDistributions)-2, 1):
                if (i != j) :
                    ...
                    for key in crossPartitionFeatures.keys():
                        crossPartitionIterationsBasedStatisticsAttributes[len(attributes) + 
                                                                              len(crossPartitionIterationsBasedStatisticsAttributes)] = crossPartitionFeatures.get(key)
    ...
    def calculateScoreDistributionStatisticsOverMultipleSetsAndIterations(
             self, currentIteration, iterationsEvaluationInfo1,
            iterationsEvaluationInfo2,  targetClassIndex,
        ...
        instanceScoreTargetClassGroup1 = self.init_list(len(currentIterationScoreDistGroup1.keys()) , 0.0)
        instanceScoreTargetClassGroup2 = self.init_list(len(currentIterationScoreDistGroup2.keys()) , 0.0) 
        instanceDeltaScoreTargetClass = self.init_list(len(currentIterationScoreDistGroup1.keys()) , 0.0)
        deltaBetweenGroups= []
        ...
        #max
        maxDeltaScoreDist = attributeInfo("maxDeltaScoreDist_"+identifier+"_iteration_"+currentIteration, "Numeric"
                        , max(deltaBetweenGroups), -1) 
        partitionBasedStatisticsAttributes[len(partitionBasedStatisticsAttributes)] = maxDeltaScoreDist
        #min
        minDeltaScoreDist = attributeInfo("minDeltaScoreDist_"+identifier+"_iteration_"+currentIteration, "Numeric"
                        , min(deltaBetweenGroups), -1) 
        partitionBasedStatisticsAttributes[len(partitionBasedStatisticsAttributes)] = minDeltaScoreDist
        #mean
        meanDeltaScoreDist = attributeInfo("meanDeltaScoreDist_"+identifier+"_iteration_"+currentIteration, "Numeric"
                        , np.mean(deltaBetweenGroups), -1) 
        partitionBasedStatisticsAttributes[len(partitionBasedStatisticsAttributes)] = meanDeltaScoreDist 
        #std
        stdDeltaScoreDist = attributeInfo("stdDeltaScoreDist_"+identifier+"_iteration_"+currentIteration, "Numeric"
        ...
        #p-50
        medianDeltaScoreDist = attributeInfo("medianDeltaScoreDist_"+identifier+"_iteration_"+currentIteration, "Numeric"
        ...
        TTestStatistic = stats.ttest_ind(instanceScoreTargetClassGroup1,instanceScoreTargetClassGroup2).statistic 
    ...
    def calculateScoreDistributionStatisticsOverMultipleIterations(self, currentIteration,
        ...
        for i in range(0, len(iterationsEvaluationInfo), 1):
            new_list = []
            confidenceScoreDifferencesPerSingleIteration[i] = new_list
            ...
            for numOfIterationsBack in self.numOfIterationsBackToAnalyze:
                ...
                if (currentIteration - (i-1) <= numOfIterationsBack) :
                    for j in currentIterationScoreDistribution.keys() :
                        if (not j in confidenceScoreDifferencesPerInstance) :
                            hashmap = {}
                            confidenceScoreDifferencesPerInstance[j] = hashmap 
                         
                        if (not numOfIterationsBack in confidenceScoreDifferencesPerInstance.get(j)) :
        ...
        for numOfIterationsBack in self.numOfIterationsBackToAnalyze:
            if (currentIteration >= numOfIterationsBack) :
                ...
                maxAvgAtt = attributeInfo(numOfIterationsBack + "iterationsAverageOfMaxDelta" + "_" + identifier, "Numeric", iteraionTempDSMax.getMean(), -1) 
                maxStdevAtt = attributeInfo(numOfIterationsBack + "iterationsStdevOfMaxDelta" + "_" + identifier, "Numeric", iteraionTempDSMax.getStandardDeviation(), -1) 
                ...
                minAvgAtt = attributeInfo(numOfIterationsBack + "iterationsAverageOfMinDelta" + "_" + identifier, "Numeric", iterationTempDSMin.getMean(), -1) 
                minStdevAtt = attributeInfo(numOfIterationsBack + "iterationsStdevOfMinDelta" + "_" + identifier, "Numeric", iterationTempDSMin.getStandardDeviation(), -1) 
                ...
                avgAvgAtt = attributeInfo(numOfIterationsBack + "iterationsAverageOfAvgDelta" + "_" + identifier, "Numeric", iterationTempDSAvg.getMean(), -1) 
                avgStdevAtt = attributeInfo(numOfIterationsBack + "iterationsStdevOfAvgDelta" + "_" + identifier, "Numeric", iterationTempDSAvg.getStandardDeviation(), -1) 
                ...
                stdevAvgAtt = attributeInfo(numOfIterationsBack + "iterationsAverageOfStdevDelta" + "_" + identifier, "Numeric", iterationTempDSStdev.getMean(), -1) 
                stdevStdevAtt = attributeInfo(numOfIterationsBack + "iterationsStdevOfStdevDelta" + "_" + identifier, "Numeric", iterationTempDSStdev.getStandardDeviation(), -1) 
                ...
                medianAvgAtt = attributeInfo(numOfIterationsBack + "iterationsAverageOfMedianDelta" + "_" + identifier, "Numeric", iterationTempDSMedian.getMean(), -1) 
                medianStdevAtt = attributeInfo(numOfIterationsBack + "iterationsStdevOfMedianDelta" + "_" + identifier, "Numeric", iterationTempDSMedian.getStandardDeviation(), -1) 
                ...
                for instanceID in confidenceScoreDifferencesPerInstance.keys():
                    try:
                        instanceTempDSMax.append(max(confidenceScoreDifferencesPerInstance.get(instanceID).get(numOfIterationsBack))) 
                        instanceTempDSMin.append(min(confidenceScoreDifferencesPerInstance.get(instanceID).get(numOfIterationsBack))) 
                        instanceTempDSAvg.append(np.mean(confidenceScoreDifferencesPerInstance.get(instanceID).get(numOfIterationsBack))) 
                        instanceTempDSStdev.append(statistics.stdev(confidenceScoreDifferencesPerInstance.get(instanceID).get(numOfIterationsBack))) 
                ...
                maxAvgPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesAverageOfMaxDelta" + "_" + identifier, "Numeric", instanceTempDSMax.getMean(), -1) 
                maxStdevPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesStdevOfMaxDelta" + "_" + identifier, "Numeric", instanceTempDSMax.getStandardDeviation(), -1) 
                ...
                minAvgPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesAverageOfMinDelta" + "_" + identifier, "Numeric", instanceTempDSMin.getMean(), -1) 
                minStdevPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesStdevOfMinDelta" + "_" + identifier, "Numeric", instanceTempDSMin.getStandardDeviation(), -1) 
                ...
                avgAvgPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesAverageOfAvgDelta" + "_" + identifier, "Numeric", instanceTempDSAvg.getMean(), -1) 
                avgStdevPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesStdevOfAvgDelta" + "_" + identifier, "Numeric", instanceTempDSAvg.getStandardDeviation(), -1) 
                ...
                stdevAvgPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesAverageOfStdevDelta" + "_" + identifier, "Numeric", instanceTempDSStdev.getMean(), -1) 
                stdevStdevPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesStdevOfStdevDelta" + "_" + identifier, "Numeric", instanceTempDSStdev.getStandardDeviation(), -1) 
                ...
                medianAvgPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesAverageOfMedianDelta" + "_" + identifier, "Numeric", instanceTempDSMedian.getMean(), -1) 
                medianStdevPerInstanceAtt = attributeInfo(numOfIterationsBack + "instancesStdevOfMedianDelta" + "_" + identifier, "Numeric", instanceTempDSMedian.getStandardDeviation(), -1) 
        ...
        for numOfIterationsBack in self.numOfIterationsBackToAnalyze:
            if (currentIteration >= numOfIterationsBack) :
                ...
                i = 0.0
                while i < 1:
                    generalPercentageScoresDiffHistogramByIteration.get(numOfIterationsBack)[int(math.round(i/self.histogramItervalSize))] = 0.0
                ...
                for  i in earlierIterationScoreDistribution.keys():
                    histogramIndex = int(math.round(earlierIterationScoreDistribution.get(i)[targetClassIndex]/self.histogramItervalSize)) 
        ...
        currentIterationTargetClassScoreDistributions = self.init_list(len(scoreDistributions.keys()),0.0)
        ...
        for i in range(0, len(iterationsEvaluationInfo), 1):
            ...
            tempIterationTargetClassScoreDistributions = self.init_list(len(currentIterationTargetClassScoreDistributions), 0.0)#CRITICAL: because of the differences in the sizes of the unlabeled sets, we always use the one of the current iteration (which is smallest and shared by all previous iterations)
            ...
            TTestStatistic = stats.ttest_ind(currentIterationTargetClassScoreDistributions,tempIterationTargetClassScoreDistributions).statistic 
        ...
        for numOfIterationsBack in self.numOfIterationsBackToAnalyze:
            if (currentIteration >= numOfIterationsBack) :
                ...
                for i in range(currentIteration-numOfIterationsBack , currentIteration, 1):
        ...
        for numOfIterationsBack in self.numOfIterationsBackToAnalyze:
            if (currentIteration >= numOfIterationsBack) :
                ...
                tempIterationTargetClassScoreDistributions = self.init_list(len(scoreDistributions.keys()),0.0)
                ...
                for threshold in self.confidenceScoreThresholds:
                    counter = 0.0 
                    for i in range(0, len(currentIterationTargetClassScoreDistributions), 1):
    ...
    def calculateGeneralScoreDistributionStatistics(self , unlabeledSamplesDataset, labeledSamplesDataset, scoreDistributions,
        ...
        random.seed(int(properties.randomSeed))
        ...
        i = 0.0
        while i < 1:
            generalPercentageByScoreHistogram[int(math.round(i/self.histogramItervalSize))] = 0.0
            i += self.histogramItervalSize
        
        for i in scoreDistributions.keys():
            histogramIndex = int(math.round(scoreDistributions.get(i)[targetClassIndex]/self.histogramItervalSize)) 
        ...
        keysetPositions = scoreDistributions.keys() 
        ...
        if (len(scoreDistributions.keys()) < desiredNumberOfSamplesInSet) :
            ...
            scoresListToPull = copy.deepcopy(scoreDistributions.keys())
            ...
            for pos in scoresListToPullRandomSet:
                ...
                samplesConfidenceScoreValues[pos] = scoreDistributions.get(pos)[targetClassIndex] 
        ...
        uniformRandomVals = self.init_list(len(samplesConfidenceScoreValues), 0.0)
        for i in range(0, len(samplesConfidenceScoreValues), 1):
            uniformRandomVals[i] = random.uniform(0.0, 1.0)
         

        #Now we create the objects needed for the Kolmogorov-Smirnov test
        #uniformRandomVals - the random values
        #samplesConfidenceScoreValues.values() - the values we sampled from the results
        alpha = 0.05
        for pval in pValuesList:
            ...
            for ci in unlabeledSamplesDataset.getAllColumns(False):
                try:
                    ...
                    # It is important to note that we can't use paired-t test because the samples are not paired.*/
                        belowThresholdValues = self.init_list(len(scoreDistributions.keys())-len(indicesOfIndicesOverTheThreshold), 0.0)
                        belowThresholdCounter = 0 
                        aboveThresholdValues = self.init_list(len(indicesOfIndicesOverTheThreshold) , 0.0)
                        aboveThresholdCounter = 0 
                        values = ci.getColumn().getValues() 
                        for i in range(0, len(values), 1):
                            if (i in indicesOfIndicesOverTheThreshold) :
                    ...
                    if (ci.getColumn().getType() == "Discrete") :
                        belowThresholdValues = self.init_list(len(scoreDistributions.keys())-len(indicesOfIndicesOverTheThreshold) , 0)
                        belowThresholdCounter = 0 
                        aboveThresholdValues = self.init_list(len(indicesOfIndicesOverTheThreshold) , 0)
                        aboveThresholdCounter = 0 
                        values = ci.getColumn().getValues() 
        ...
        numOfTargetClassInstancesInTrainingData = 0.0 
        numOfNonTargetClassInstancesInTrainingData = 0.0 
        for i in range(0, len(labeledSamplesDataset.getNumOfRowsPerClassInTrainingSet()), 1):
        ...
        for threshold in self.confidenceScoreThresholds:
            instancesAboveThreshold = 0.0 
            instancesBelowThreshold = 0.0
            for i in scoreDistributions.keys():
                if (scoreDistributions.get(i)[targetClassIndex] >= threshold) :
    ...
    def setNumOfIterationsBackToAnalyze(self, numOfIterationsBackToAnalyze) :
        self.numOfIterationsBackToAnalyze = numOfIterationsBackToAnalyze 
